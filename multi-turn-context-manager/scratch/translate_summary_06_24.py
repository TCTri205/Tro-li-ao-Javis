...
csv_path = r"D:\VJ\Tro-li-ao-Javis\multi-turn-context-manager\reports\tests\test_summary_06_24.csv"
...
def translate_text(text: str) -> str:
    ...
    # Check for simple mock metrics or logs in English
    if "Orchestrator raises asyncio.TimeoutError" in text:
        return "Orchestrator kích hoạt lỗi asyncio.TimeoutError (Quá thời gian chờ)"
    if "Breaker transitions" in text:
        return text.replace("Breaker transitions:", "Các chuyển đổi trạng thái Circuit Breaker:").replace("call_count", "số lượt gọi")
    if "Results: A=" in text:
        return text.replace("Results:", "Kết quả:")
    
    system_prompt = (
        "You are an expert translator translating Japanese/English to Vietnamese.\n"
        "Please strictly adhere to the following rules:\n"
        "1. Translate Japanese personal names using their standard Romaji transliteration (e.g. '島田' -> 'Shimada', '中原' -> 'Nakahara', '中原凛花' -> 'Nakahara Rinka', '中岡' -> 'Nakaoka', '山下' -> 'Yamashita', '伊藤' -> 'Ito', '辻' -> 'Tsuji', '石原' -> 'Ishihara', 'サカモト' -> 'Sakamoto', 'クマガイ' -> 'Kumagai', 'カセ/加瀬' -> 'Kase', '横堀' -> 'Yokohori', '島田' -> 'Shimada', '熊田' -> 'Kumada', '佐藤太郎' -> 'Sato Taro', '鈴木花子' -> 'Suzuki Hanako'). Do NOT use Sino-Vietnamese names (e.g. do NOT use 'Trung Nguyên', 'Đảo Điền') and do NOT use hybrid characters.\n"
        "2. Translate 'AJテクノロジーズ' as 'AJ Technologies'.\n"
        "3. Translate 'Multiple Answers' as 'Nhiều phản hồi' or 'Nhiều câu trả lời'.\n"
        "4. Keep the exact structural format (starting with 'T1:', 'T2:', preserving the pipes '|' and line breaks).\n"
        "5. Do NOT hallucinate or change names.\n"
...
def main():
    ...
    try:
        ...
        print(f"Could not find required columns in header! Error: {e}")
        sys.exit(1)
    ...
    for i in range(1, len(rows)):
        ...
        version_val = row[version_idx]
        actual_val = row[actual_idx]
        
        # Only translate V4 rows
        if version_val == "V4":
            print(f"\n--- Translating Row {i} (Scenario: {row[2]}) ---")
            print(f"Original: {actual_val[:120]}...")
            
            try:
                translated_val = translate_text(actual_val)
                print(f"Translated: {translated_val[:120]}...")
                row[translation_idx] = translated_val
            except Exception as e:
                print(f"Error translating row {i}: {e}")
                row[translation_idx] = f"[Translation Failed] {actual_val}"
        
        new_rows.append(row)
        # Small sleep to be nice to API limits
        time.sleep(0.5)
    ...
    print("\nDone! Successfully translated V4 actual results.")
