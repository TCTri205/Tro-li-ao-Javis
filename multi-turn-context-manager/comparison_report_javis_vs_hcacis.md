...
| **HCACIS** | **Rủi ro im lặng khi gọi Structured Output thất bại** | `llm_client.py` dòng 72-78 | Khi việc gọi structured output bị lỗi hoặc timeout, hàm bắt ngoại lệ và trả về `schema()` (một thực thể Pydantic mặc định). Điều này khiến luồng chạy tiếp với các thuộc tính mặc định (`is_followup=False`, `relation_type='none'`, `intent_category='pure_llm'`, `needs_retrieval='none'`) thay vì báo lỗi, dẫn đến định tuyến sai hoàn toàn trong âm thầm. |
...
4. **Ràng buộc kiểu dữ liệu bằng Pydantic**: Refactor lại các cấu trúc trao đổi thông tin chính giữa các Engine và Orchestrator của Javis sang Pydantic Models để tăng tính Type Safety.
