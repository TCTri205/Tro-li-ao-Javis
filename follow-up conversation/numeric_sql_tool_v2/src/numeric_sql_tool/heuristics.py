...
def _is_semantic(query: str) -> bool:
    ...
    # Bypass for strong numeric queries
    if re.search(r"何件|いくつありましたか|合計時間|何秒|平均|一番短|一番長", query):
        return False
...
def enforce_intent_invariants(intent: NumericIntent, query: str) -> NumericIntent:
    ...
    # Invariant 2: If the LLM mistakenly put the keyword in entity_filter but target is meeting-level, move it to context_filter
    if intent.target in {"meeting_count", "duration_seconds"} and intent.entity_filter:
        if not intent.context_filter or "月" in intent.context_filter or "年" in intent.context_filter or "日" in intent.context_filter:
            intent.context_filter = intent.entity_filter
            intent.entity_filter = None
