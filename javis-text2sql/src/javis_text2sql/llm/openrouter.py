...
class OpenRouterClient(LLMClient):
    def __init__(
        ...
        if not self.api_keys:
            raise ValueError("At least one non-empty OpenRouter API key must be provided")
        self.model = model
    ...
    async def _request_with_rotation(
            ...
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    if response.status_code == 429:
                        logger.warning(
                            f"OpenRouter API key ({key_display}) returned 429 Rate Limit. Rotating to next key."
                        )
                ...
                logger.warning(
                    f"OpenRouter request failed with key ({key_display}): {e}. Retrying with next key if available."
                )
                last_exception = e
                if (attempt + 1) % num_keys == 0:
        ...
        raise RuntimeError(
            f"All {num_keys} OpenRouter API keys failed after {max_attempts} attempts. Last error: {last_exception}"
        )
