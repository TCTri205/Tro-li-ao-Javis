...
class HCACIS:
    def __init__(self, detector_llm: LLMClient, generator_llm: LLMClient, db_pool: Any):
        ...
        # Planner uses detector_llm (Llama-3.3-70b-versatile) internally for highly accurate numeric intent extraction
    ...
    def _build_graph(self):
        ...
        async def detect_node(state: GraphState):
            turn_state = state["turn_state"]
            active_context = {
                "active_entities": [e.model_dump() for e in turn_state.active_entities],
                "retrieved_data": turn_state.retrieved_data
            }
            detector_output = await self.detector.detect(state["current_query"], turn_state.messages, active_context)
            turn_state = self.memory_manager.update_context(turn_state, detector_output)
        ...
        async def plan_node(state: GraphState):
            ...
            # Post planning: dynamically register the active entity to Graph DB
            if turn_state.active_entities:
                self.memory_manager.register_entity(turn_state, turn_state.active_entities[0])
                
            return {"turn_state": turn_state}

        async def generate_node(state: GraphState):
            turn_state = state["turn_state"]
            turn_state = await self.generator.generate(turn_state)
