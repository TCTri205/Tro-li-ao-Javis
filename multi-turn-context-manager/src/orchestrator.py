    ...
    async def _generate_llm_answer_with_self_check(self, original_query: str, rewritten_query: str, pipeline: str, payload: dict, summary_context: dict = None) -> tuple[str, str, bool, int]:
        ...
        if pipeline == "MODEL":
            system_prompt = (
                ...
                "ユーザーの質問に対して、あなたの知識に基づいて親切に回答してください。\n"
                "回答は日本語で行ってください。\n"
            )
        else:
            system_prompt = (
                ...
                "5. 回答は日本語で行ってください。\n"
                "6. 登場人物や企業の「立場」（例：誰が電話をかけた発信側か、誰が電話を受けた受信側か）について問われた場合、コンテキストの会話の冒頭の挨拶表現から、どちらが発信元・受信元であるかを注意深く論理的に判定してください（「はい、〇〇です」と応答した話者が「受信側」、「お世話になります。〇〇の〇〇です」と名乗った話者が「発信側」です）。呼び出されたが不在の人物（例：イシハラさん）は電話に出ておらず「受信側」の対話相手ではありません。発信側・受信側の名前や会社名を正しく対応付けて、「発信側は山下（AJテクノロジーズ）」、「受信側はマルケン（受付）」のように明確に区別して回答してください。間違えて所属や立場を逆に解釈しないように注意してください。\n"
                ...
                "9. 会話の参加者がどうするか（今後の行動や予定）について問われた場合、その人物が通話の途中で述べた希望や意図（例：内見をしたい等）だけでなく、通話の最後で合意した最終的な対応（例：担当者からの折り返しの電話を待つことにした、承諾した、承知した、折り返しを了解した等）についても漏れなく含めて回答してください。\n"
                "10. 対話ログ内の「Nữ」「Nam」といった話者ラベルは、それぞれ女性・男性の対話者（電話の受付担当者など）を指す一般的な表示です。「Nữ」などの話者が、探している特定の人物（例：中原凛花さんなどの対象従業員）とは異なる第三者の受付担当者であることを正しく認識し、混同しないように回答してください。\n"
            )
