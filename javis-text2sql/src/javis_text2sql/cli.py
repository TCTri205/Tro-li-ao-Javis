...
async def _cmd_seed(args: argparse.Namespace) -> None:
    ...
    print(json.dumps({"seeded_entity_aliases": count}, ensure_ascii=False, indent=2))


async def _cmd_ingest_samples(args: argparse.Namespace) -> None:
    settings = Settings.from_env()
    database_url = args.database_url or require_database_url(settings.database_url)
    if not args.fixture_llm:
        raise RuntimeError("Only --fixture-llm is available without a production LLM adapter")
    pool = await create_pool(database_url)
    try:
        meeting_ids = await ingest_sample_files(pool, FixtureLLMClient(), sample_dir=Path(args.sample_dir))
    finally:
        await pool.close()
...
async def _cmd_eval(args: argparse.Namespace) -> None:
    report = await evaluate_sample_fixtures(sample_dir=Path(args.sample_dir), llm_client=FixtureLLMClient())
    if args.output:
