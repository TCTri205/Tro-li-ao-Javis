...
async def extract_numeric_intent(question: str, llm_client: LLMClient | None = None) -> NumericIntent:
    ...
    try:
        ...
        if fallback.operator in {"max", "min", "avg", "count"} and result.operator == "sum":
        ...
        # Preserve turn-level fields from heuristic if LLM doesn't set them
        if result.speaker_filter is None and fallback.speaker_filter is not None:
            result.speaker_filter = fallback.speaker_filter
        if result.entity_filter is None and fallback.entity_filter is not None:
            result.entity_filter = fallback.entity_filter
        # If heuristic detected turn-level target, trust heuristic over LLM
        if fallback.target in {"turn_count", "turn_duration", "mention_count", "speaker_name"}:
            result.target = fallback.target
            result.operator = fallback.operator
...
async def run_numeric_pipeline(
    ...
    *,
    ...
    # params $1..$4 are the standard meeting-scope params
...
def _meeting_where_clause() -> str:
    """Standard WHERE for queries on transcripts table (params $1..$4)."""
    return (
...
def _turn_where_clause() -> str:
    """Standard WHERE for queries that JOIN chunks_turn + transcripts (params $1..$4)."""
    return (
        "($1::uuid IS NULL OR t.user_id = $1::uuid) "
        "AND ($2::date IS NULL OR t.meeting_date >= $2::date) "
        "AND ($3::date IS NULL OR t.meeting_date <= $3::date) "
...
def _numeric_where_clause() -> str:
    return _meeting_where_clause()


# ── SQL builder (no DB needed, for export/testing) ───────────────────────────

def build_numeric_sql(intent: NumericIntent) -> str | None:
    """Return the SQL that run_numeric_pipeline would execute (params $1..$4, optional $5)."""
    if intent.operator in {"skip", "none"} or intent.target in {"none", "time_start_sec"}:
        return None
    ...
    if intent.target == "mention_count":
        entity = intent.entity_filter or ""
        entity_clause = f"AND ct.text ILIKE '%{entity}%'" if entity else ""
        return (
            "SELECT COUNT(*) AS value "
            "FROM chunks_turn ct "
            "JOIN transcripts t ON t.id = ct.transcript_id "
            f"WHERE {_turn_where_clause()} "
            f"{entity_clause}"
        )

    if intent.target == "turn_count":
        speaker_clause = f"AND ct.speaker = '{intent.speaker_filter}'" if intent.speaker_filter else ""
        return (
            "SELECT COUNT(*) AS value "
            "FROM chunks_turn ct "
            "JOIN transcripts t ON t.id = ct.transcript_id "
    ...
    if intent.target == "turn_duration":
        speaker_clause = f"AND ct.speaker = '{intent.speaker_filter}'" if intent.speaker_filter else ""
        agg = "AVG" if intent.operator == "avg" else ("MAX" if intent.operator == "max" else "SUM")
        return (
            f"SELECT ROUND({agg}(ct.time_end_sec - ct.time_start_sec)::numeric, 2) AS value "
            "FROM chunks_turn ct "
            "JOIN transcripts t ON t.id = ct.transcript_id "
            f"WHERE {_turn_where_clause()} "
            f"{speaker_clause}"
    ...
    if intent.target == "speaker_name":
        if intent.operator == "max" and intent.group_by == "speaker":
        ...
        elif intent.operator == "min":
            # Speaker with shortest single turn
            return (
                "SELECT ct.speaker AS value, MIN(ct.time_end_sec - ct.time_start_sec) AS duration "
                "FROM chunks_turn ct "
                "JOIN transcripts t ON t.id = ct.transcript_id "
                f"WHERE {_turn_where_clause()} "
                "GROUP BY ct.speaker "
    ...
    # ── Meeting-level targets ─────────────────────────────────────────────────

    where = _meeting_where_clause()
...
async def _run_numeric_query(
    ...
    *,
    ...
    if intent.target == "mention_count":
        ...
        val = float(rows[0]["value"]) if rows else 0.0
        return [NumericRow(value=val)], sql
    ...
    if intent.target == "turn_duration":
        ...
        rows = await _fetch_rows(conn, sql, params, user_id, statement_timeout_ms)
        val = float(rows[0]["value"]) if rows and rows[0]["value"] is not None else 0.0
        return [NumericRow(value=val)], sql
    ...
    if intent.target == "speaker_name":
        ...
        speaker = rows[0].get("value", "")
        # Return numeric 0 but carry speaker name in metadata
        return [NumericRow(value=0.0, metadata={"speaker": speaker, "raw": dict(rows[0])})], sql
